...
if "%maxspeed%"=="1" (
	...
	for /f "usebackq delims=" %%T in ("%~dpn1.bfp") do (echo;echo:%%T)>> "%~dpn1.bfp.bat"
	echo.^)^>^>x>> "%~dpn1.bfp.bat"
) else (
	echo @echo off^&for %%%%B in ^(>>"%~dpn1.bfp.bat"
	for /f "usebackq delims=" %%T in ("%~dpn1.bfp") do (echo;%%T>> "%~dpn1.bfp.bat")
	echo.^)do echo:%%%%B^>^>x>> "%~dpn1.bfp.bat"
)


echo certutil -f -decode "%%CD%%\x" "%%CD%%\b"^>nul>> "%~dpn1.bfp.bat"
echo del x /q^>nul>> "%~dpn1.bfp.bat"


::if "%seven-zip%"=="1" (
	::echo %seven_command% e -o"%%TMP%%" -y "%%CD%%\x.bfp" 2^>nul 1^>nul&del x.bfp /q^>nul>> "%~dpn1.bfp.bat"
::) else (
echo expand b "%%TMP%%\%%~n0%~x1"^>nul>> "%~dpn1.bfp.bat"
echo del b /q^>nul>> "%~dpn1.bfp.bat"
::)
...
set first_line_bfp=
for /f "usebackq delims=" %%A in ("%~f2") do (
	if not defined first_line_bfp 

for /f "usebackq eol=; delims=" %%A in ("%~f2") do (
	if /i "%%~A"=="[BatchFilePacker]"  & exit /b
...
if not defined first_line_bfp (
	echo.Invalid .bfp file.
	exit /b 1
)
	
	
:process_file
makecab "%name%" "%name%__"
echo Const fsDoOverwrite     = true > "%TMP%\BFP.vbs"
echo Const fsAsASCII         = false >> "%TMP%\BFP.vbs"
echo Const adTypeBinary      = 1 >> "%TMP%\BFP.vbs"
echo Set objStream = CreateObject("ADODB.Stream") >> "%TMP%\BFP.vbs"
echo objStream.Type = adTypeBinary >> "%TMP%\BFP.vbs"
echo objStream.Open() >> "%TMP%\BFP.vbs"
echo objStream.LoadFromFile("%name%__") >> "%TMP%\BFP.vbs"
echo Set objXML = CreateObject("MSXml2.DOMDocument") >> "%TMP%\BFP.vbs"
echo Set objDocElem = objXML.createElement("Base64Data") >> "%TMP%\BFP.vbs"
echo objDocElem.dataType = "bin.base64" >> "%TMP%\BFP.vbs"
echo objDocElem.nodeTypedValue = objStream.Read() >> "%TMP%\BFP.vbs"
...
echo Set objFileOut = objFSO.CreateTextFile("%name%_b64__", fsDoOverwrite, fsAsASCII) >> "%TMP%\BFP.vbs"
echo objFileOut.Write objDocElem.text >> "%TMP%\BFP.vbs"
echo objFileOut.Close() >> "%TMP%\BFP.vbs"
